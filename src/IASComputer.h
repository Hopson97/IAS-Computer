...
const std::unordered_map<Word, std::string> opcodeString{
    ...
    {6, "Set program counter to the memory address, given the accumulator's "
        "value is positive"},
    {7, "Exit the program."},
};
...
class IASComputer {
  ...
  public:
    ...
    const RegType *getFirstRegister() const
    {
        return &m_registers.accumulator;
    }
    RegType getProgramCounter() const
    {
        return m_registers.programCounter;
    }
    RegType getAddressRegister() const
    {
        return m_registers.memoryAddress;
    }
